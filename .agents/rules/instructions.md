...
- **Match Feature:** The `FelfedezesSimulator.tsx` has been successfully refactored. The swipe deck logic lives in `src/components/match/ProfileCard.tsx`, and the UI elements are in `src/components/ui/`. The swipe view is correctly set as the default view.
